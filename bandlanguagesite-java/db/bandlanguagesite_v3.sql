...
INSERT INTO `bls_dept_script` VALUES (8, 2, 8);
INSERT INTO `bls_dept_script` VALUES (9, 2, 9);
INSERT INTO `bls_dept_script` VALUES (10, 2, 10);
INSERT INTO `bls_dept_script` VALUES (11, 2, 11);
INSERT INTO `bls_dept_script` VALUES (12, 2, 12);
INSERT INTO `bls_dept_script` VALUES (13, 2, 13);
INSERT INTO `bls_dept_script` VALUES (14, 2, 14);
INSERT INTO `bls_dept_script` VALUES (15, 2, 15);
INSERT INTO `bls_dept_script` VALUES (16, 2, 16);
INSERT INTO `bls_dept_script` VALUES (17, 2, 17);
COMMIT;
...
INSERT INTO `bls_dept_sentence` VALUES (36, 2, 36);
INSERT INTO `bls_dept_sentence` VALUES (37, 2, 37);
INSERT INTO `bls_dept_sentence` VALUES (38, 2, 38);
INSERT INTO `bls_dept_sentence` VALUES (39, 2, 39);
INSERT INTO `bls_dept_sentence` VALUES (40, 2, 40);
INSERT INTO `bls_dept_sentence` VALUES (41, 2, 41);
INSERT INTO `bls_dept_sentence` VALUES (42, 2, 42);
INSERT INTO `bls_dept_sentence` VALUES (43, 2, 43);
INSERT INTO `bls_dept_sentence` VALUES (44, 2, 44);
INSERT INTO `bls_dept_sentence` VALUES (45, 2, 45);
INSERT INTO `bls_dept_sentence` VALUES (46, 2, 46);
INSERT INTO `bls_dept_sentence` VALUES (47, 2, 47);
INSERT INTO `bls_dept_sentence` VALUES (48, 2, 48);
INSERT INTO `bls_dept_sentence` VALUES (49, 2, 49);
INSERT INTO `bls_dept_sentence` VALUES (50, 2, 50);
INSERT INTO `bls_dept_sentence` VALUES (51, 2, 51);
INSERT INTO `bls_dept_sentence` VALUES (52, 2, 52);
INSERT INTO `bls_dept_sentence` VALUES (53, 2, 53);
INSERT INTO `bls_dept_sentence` VALUES (54, 2, 54);
COMMIT;
...
INSERT INTO `bls_dept_word` VALUES (79, 3, 84);
COMMIT;
...
CREATE TABLE `bls_node` (
  ...
  `package` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '该类所在的路径、包名',
  ...
  `status` int NOT NULL COMMENT '0:禁用 1：待开发 2： 开发中 3：待审核 4：待发布 5：已发布',
  PRIMARY KEY (`node_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `bls_rule` (
  ...
  `chinesename` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '中文名称',
...
-- ----------------------------
-- Table structure for bls_rule_node
-- ----------------------------
DROP TABLE IF EXISTS `bls_rule_node`;
CREATE TABLE `bls_rule_node` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `rule_id` bigint NOT NULL COMMENT '规则id',
  `node_id` bigint NOT NULL COMMENT '节点id',
  PRIMARY KEY (`id`)
...
INSERT INTO `bls_sentence` VALUES (36, '工具调用句型一', '<工具名称>和<接口名称>是指某个工具以及它所提供的功能接口的中文名称，这些名称在平台中都是唯一的。句型中如果调用的接口不需要参数，则<参数赋值短语>可以省略。', '执行 <工具名称> ，(其中，(<参数赋值短语> ，) + )? 得到 <接口名称> (、<接口名称>)*', 3, 3, '2021-05-27 09:58:33', 1);
INSERT INTO `bls_sentence` VALUES (37, '工具调用句型二', '考虑到对于部分工具接口而言，其所需参数可能较多，如果借助前面的文法来进行描述，则用户需要在<参数赋值短语>中指定大量的参数，这会使得语句过长难以阅读。为此实现了句型二。如果某个接口所需的参数，与某个对象的各属性能够对应，则可以直接指定该对象作为参数，从而完成接口的调用。', '(把 <对象名称> 作为参数，)? 执行 <工具名称> 的 <接口名称> (、<接口名称>)*', 3, 3, '2021-05-27 10:01:29', 1);
INSERT INTO `bls_sentence` VALUES (38, '修改对象的属性值', '该操作只会影响到功能接口返回的数据对象，并不会影响到工具内部的数据。工具内部的数据只能通过工具对外开放的接口才能够进行修改。举例：把 论文A 的 作者 改成 ‘张三’', '把 <对象名称> 的 <属性名称> 改成 <值>', 3, 3, '2021-05-27 10:03:33', 1);
INSERT INTO `bls_sentence` VALUES (39, '新建对象', '在集成帮语中，除了调用工具功能接口来获取对象外，还可以自己创建新对象，以方便进行功能集成。创建的对象也可作为工具接口的输入参数。举例：新建 论文A ，其中，标题 等于 帮语综述，作者 等于 张三', '新建 <对象名称>  ，其中 (，<参数赋值短语>)+', 3, 3, '2021-05-27 10:05:25', 1);
INSERT INTO `bls_sentence` VALUES (40, '删除对象', '如果集成帮语执行过程中某个对象后续不再需要，则可以使用删除操作来删除该对象。举例：删除 论文集合', '删除 <对象名称>', 3, 3, '2021-05-27 10:07:16', 1);
INSERT INTO `bls_sentence` VALUES (41, '删除集合对象中的部分元素', '对于集合对象，集成帮语同样允许用户删除其中符合条件的元素，类似SQL中的delete操作。区别于“删除对象”句型，“删除记录”句型是删除某个集合对象中的部分元素，即使元素全部删除，该对象在语境中依然存在。举例：去掉 论文集合 中 作者 为 ‘张三’的 元素', '去掉 <对象名称> 中 (<条件判断语句>)? 的 元素', 3, 3, '2021-05-27 10:08:18', 1);
INSERT INTO `bls_sentence` VALUES (42, '将集合对象按照某个属性的值进行排序', '举例：根据 发表时间 将 论文集合 进行 升序排序', '根据 <属性名称> 将 <对象名称> 进行 (升序排序 | 降序排序)', 3, 3, '2021-05-27 10:09:07', 1);
INSERT INTO `bls_sentence` VALUES (43, '将集合对象按照某个属性的值进行分组', '该操作完成后所有元素依然在同一个集合对象中，只是属性值相同的会排列在一起。举例：将 论文集合 按照 作者 进行分组', '将 <对象名称> 按照 <属性名称> 进行分组', 3, 3, '2021-05-27 10:11:34', 1);
INSERT INTO `bls_sentence` VALUES (44, '将对象重命名', '工具功能接口所返回的数据，会默认以接口名称为key保存成为对象。如果集成帮语运行过程中有多个key相同的对象，开发者可使用重命名操作修改最近使用的对象的名称。语句执行后会将对象的key修改为新值，但不会修改其实际内容。举例：把 论文集合 重命名为 18级论文集合', '把 <对象名称>  重命名为 <对象名称>', 3, 3, '2021-05-27 10:12:24', 1);
INSERT INTO `bls_sentence` VALUES (45, '将某个单一对象添加到某个集合对象中', '举例：在 论文集合 中 添加 论文A', '在 <集合对象名称> 中 添加 <对象名称>', 3, 3, '2021-05-27 10:13:09', 1);
INSERT INTO `bls_sentence` VALUES (46, '把同类型、同属性的对象合并成为新的对象', '单一对象之间的合并与集合对象之间的合并相类似，故采用一种句型进行实现。实际执行过程中根据对象的类型进行自动判断。举例：把 研一学生 、研二学生、研三学生 合并成为 实验室研究生', '把 <对象名称> (、<对象名称> )+ 合并成为 <对象名称>', 3, 3, '2021-05-27 10:13:57', 1);
INSERT INTO `bls_sentence` VALUES (47, '多个集合对象根据某个属性进行连接，形成新', '举例：把 论文影响力集合 、论文参考文献评分集合 按照 论文标题 合并成为 论文详细信息集合', '把 <对象名称> (、<对象名称> )+ 按照 <属性名称> 合并成为 <对象名称>', 3, 3, '2021-05-27 10:14:59', 1);
INSERT INTO `bls_sentence` VALUES (48, '将对象内容显示在页面上', '举例：显示 B8实验室人数', '显示 <对象名称>  (的 <属性名称>)*', 3, 3, '2021-05-27 10:15:49', 1);
INSERT INTO `bls_sentence` VALUES (49, '查询某个工具所提供的功能接口，或者需要的', '显示 <工具名称> 的 输出部件|输入部件', '举例：显示 专家查询工具 的 输出部件', 3, 3, '2021-05-27 10:16:48', 1);
INSERT INTO `bls_sentence` VALUES (50, '统计某个对象集合中符合某个条件的对象的个', '如果用户希望统计集合中所有元素的数量，则不用写文法中的<条件判断>。目前，帮语的<条件判断>仅支持数字以及字符串之间的简单比较，暂时不支持更为复杂的判断操作。\n举例：统计 论文集合 中 作者 等于 张三 的 元素 的 数量', '统计 <对象名称> 中 (<条件判断语句>)? 的 元素 的 数量', 3, 3, '2021-05-27 10:17:43', 1);
INSERT INTO `bls_sentence` VALUES (51, '查询某个对象集合中符合某些条件的对象', '区别于前面的“统计”句型，该操作会将符合条件的元素集合显示在页面上，而不是显示元素的数量。举例：找出 论文集合 中 发布时间 大于 2018 的 元素', '找出 <对象名称> 中 (<条件判断语句>)? 的 元素', 3, 3, '2021-05-27 10:18:35', 1);
INSERT INTO `bls_sentence` VALUES (52, '求对象集合中某个属性的最大值最小值等特殊', '举例：求 论文集合 中 发表时间 的 最大值', '求 <对象名称> 中 <属性名称> 的 最大值|最小值|平均值|总和', 3, 3, '2021-05-27 10:20:14', 1);
INSERT INTO `bls_sentence` VALUES (53, '设定词汇固定含义', '有时候某一条语句需要多次执行，某个值需要在多个地方使用，为此设计了“设定”句型，为用户提供了类似编程语言中的宏定义的一种操作。句型执行会将对应内容以对象的方式存储在语境中，在后续的语句中可使用对应的对象名称来完成实际内容的引用。举例：设定 ‘成员人数’ 等于 统计 实验室 中 的 记录 的 数量', '设定 字符串 等于 (<语句>|字符串|数字|值)', 3, 3, '2021-05-27 10:21:03', 1);
INSERT INTO `bls_sentence` VALUES (54, '拷贝一份新数据', '在进行数据处理前，为防止后续需要再次使用原始数据，可使用数据拷贝句型将数据拷贝一份。拷贝后集成帮语运行环境中将存在一个新的对象，其内容和原始数据相一致，且不会相互关联。举例：把 论文集合 拷贝一份为 论文集合备份', '把 <对象名称>  拷贝一份为  <对象名称>', 3, 3, '2021-05-27 10:22:26', 1);
COMMIT;
...
INSERT INTO `bls_user` VALUES (2, 'liwei', '黎威', '6b63d04f80af99ae5cde482a3559c459', '(UjrELVjqwof', '13387603239', '1011529903@qq.com', NULL, '2021-05-22 15:21:45', '2021-05-22 15:21:47', 13, 1);
INSERT INTO `bls_user` VALUES (3, 'chencongying', '陈聪颖', 'c9da383357b25f5136da98e1a9ef5113', 'GDQ9j#4nBP3)', NULL, NULL, NULL, '2021-05-24 11:26:37', '2021-05-24 11:26:42', 2, 1);
...
) ENGINE=InnoDB AUTO_INCREMENT=85 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='帮语词汇表';
...
INSERT INTO `bls_word` VALUES (84, '词汇六', '词汇六描述', '词汇六例子', 1, '', 2, 2, '2021-05-27 09:54:21', 1);
COMMIT;
